...
3. look at all forms and input fields for client side validation

while were here we should take care of the rails validation methods
add validation for color...re: length too long screws up the Stockcard
if VIN present on important just update data
...
7. Needs calendar and alert system to trigger when a car has been in a certain status for a set number of days <--- blog? keep track of steps, but start with a list of status starting from oldest to newest. see if you can find a method to say like:
...
10. route authoriztions

10. Admin users page needs to be able to edit users esp: role. Dont really need an approval system, unapproved just remains 'guest' and therefore will have access to nothing.
